...
right={ <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
{loading ? ( <Loader2 className="size-3.5 animate-spin" />
) : ( <span className="size-1.5 rounded-full bg-[var(--color-bull)]" />
)} <span className="num">
{lastUpdate ? fmtTime(lastUpdate) : "—"} </span> </div>
}
>
{error ? ( <div className="tile mb-4 border-[var(--color-bear)]/30 bg-[var(--color-bear)]/5 p-3 text-sm text-[var(--color-bear)]">
Datenfehler: {error} </div>
) : null}
    ...
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <MetricTile
          ...
          tone={
            result.liquidityRiskScore > 75
              ? "bear"
              : result.liquidityRiskScore > 40
                ? "warn"
                : "bull"
          }
          value={`${result.liquidityRiskScore}/100`}
        />
      </div>
